...
interface IApiContext extends IComfyApi {
   ...
   runWorkflow: (serializedGraph: SerializedGraph, reqId?: string) => Promise<JobSnapshot | Error>;
...
export const ApiContextProvider: React.FC<{
  ...
  useEffect(() => {
   ...
   const getNodeComponents = async (): Promise<Record<string, any>> => {
      const resp = await fetchApi(API_URL.GET_REACT_COMPONENTS, { cache: 'no-store' });
      return await resp.json();
   };
