...
import { BaseNode } from '@/lib/data/nodes';
import { Handle, Position } from '@xyflow/react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';

export const CozyNode = (node: BaseNode) => {
    return () => (
        <Card className="min-w-[200px] relative rounded-sm">
            <CardHeader className='px-2 py-2 mb-2 border-b'>
                <div className="text-xs" style={{ fontSize: "10px" }}>{node.name}</div>
            </CardHeader>
                ...
                <div className='flex justify-between'>
                    ...
                    <div className="relative">
                        {node.outputs.map((output, index) => (
                            <div key={output.name} className="flex items-center justify-end" style={{ position: 'relative', top: `${index}px` }}>
                                <Handle
                                    type="source"
